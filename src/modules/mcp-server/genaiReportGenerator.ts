...
/**
 * Dynamic Report Generator using Google Gemini
 * Parallel implementation to Claude report generator
 */
export class GenaiReportGenerator {
  ...
  constructor() {
    ...
    this.model = this.client.getGenerativeModel({
      model: 'models/gemini-2.0-flash', // FREE, STABLE & OPTIMIZED
      generationConfig: {
        temperature: 0.7,
  ...
  async generateReport(
    ...
    // Step 1: Use Gemini to analyze the prompt and determine what data to fetch
    const planningPrompt = `You are an AI assistant that helps generate HRM reports. Analyze this report request and determine:
...
Respond in JSON format:
{
  ...
  "analysisType": "summary|detailed|trend|comparison"
    ...
    const planningResult = await this.model.generateContent(planningPrompt);
    ...
    try {
      ...
      plan = JSON.parse(jsonMatch ? jsonMatch[0] : planningText);
    } catch (error) {
      // Fallback plan
      plan = {
        title: 'HRM Report',
    ...
    // Step 3: Use Gemini to analyze the data and generate report content
    const reportPrompt = `Generate a comprehensive HRM report based on this data:
...
Include specific numbers, percentages, and trends from the data.`;
    ...
    const reportContent = reportResult.response.text();
  ...
  private extractSection(content: string, sectionName: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.toLowerCase().includes(sectionName.toLowerCase()) && (line.startsWith('#') || line.startsWith('##') || line.endsWith(':'))) {
        inSection = true;
        continue;
      }

      if (inSection && (line.startsWith('#') || line.startsWith('##')) && !line.toLowerCase().includes(sectionName.toLowerCase())) {
        break;
      }
...
*Report generated by HRM MCP Server with Google Gemini AI*
`;
  }
  ...
  private async generatePDF(title: string, content: string, metadata: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header
        doc.fontSize(24).font('Helvetica-Bold').text(title, { align: 'center' }).moveDown();

        // Metadata
        doc.fontSize(10).font('Helvetica').fillColor('#666666');
        doc.text(`Generated: ${new Date(metadata.generatedAt).toLocaleString()}`, { align: 'center' });
        doc.text(`AI Provider: ${metadata.aiProvider}`, { align: 'center' });
        ...
        for (const line of lines) {
          if (line.startsWith('# ')) {
            ...
            doc.fontSize(11).font('Helvetica-Bold').text(line.replace(/\*\*/g, '')).moveDown(0.2);
          } else if (line.trim()) {
        ...
        doc.strokeColor('#cccccc').lineWidth(1).moveTo(50, doc.y).lineTo(550, doc.y).stroke();
        doc.moveDown();
        doc.fontSize(9).fillColor('#666666').text('Report generated by HRM MCP Server with Google Gemini AI', { align: 'center' });
  ...
  async generateQuickReport(
    ...
    const reportPrompts: Record<string, string> = {
      ...
      attendance: 'Generate an attendance report showing attendance rates, late comers, absentee patterns, and trends',
      recruitment: 'Generate a recruitment report showing job openings, candidate pipeline, interview feedback status, and hiring metrics',
      performance: 'Generate a performance report showing review status, goal completion, and performance trends',
      leaves: 'Generate a leave report showing pending approvals, leave patterns, and leave balance analysis',
      onboarding: 'Generate an onboarding report showing new hires, incomplete onboarding items, and onboarding completion rates',
...
export const genaiReportGenerator = new GenaiReportGenerator();
