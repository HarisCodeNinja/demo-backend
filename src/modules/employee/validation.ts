...
export const createEmployeePayloadValidator = z.object({
  ...
  employmentStartDate: z.coerce.date({ error: 'Employment Start Date is required' }),
  employmentEndDate: z.coerce.date().nullish(),
...
export const updateEmployeePayloadValidator = (employeeId: string) =>
  z.object({
    userId: z.uuid('Invalid UUID format'),
    firstName: z.string({ error: 'First Name is required' }),
    lastName: z.string({ error: 'Last Name is required' }),
